...
<head>
    ...
    <script>
        ...
        $(function () {
            ...
            // 封装重置图片的函数（复用）
            function resetAlbumPic() {
                // 1. 清空文件路径显示框
                $("#location").val("");
                // 2. 清空隐藏域的图片路径（避免提交空/旧路径）
                $("#pic").val("");
                $("#lastImg").val("");
                // 3. 重置文件选择器（核心：清空已选择的文件）
                $("#i-file").val("");
                // 4. 还原封面预览图为默认图
                var defaultImg = "../../img/gallery-photo/thumb-3.jpg"; // 你的默认图路径
                $("#albumImg").attr("src", defaultImg);
                // 5. 还原预览图的链接
                $(".example-image-link").attr("href", "../../img/gallery-photo/image-3.jpg");
            }
            ...
            $("[modify]").click(function () {
                ...
                $.ajax({
                    ...
                    success: function (jsonObj) {
                        ...
                        // $("#ppic").val(jsonObj.pic);
                        $("#pcompany").val(jsonObj.company);
                        $("#plang").val(jsonObj.lang);
                        $("#ppdate").val(jsonObj.pdate);


                        // 图片回显（关键：更新预览图src）
                        if (jsonObj.pic) {
                            var fullPicUrl = "${filePath}" + jsonObj.pic; // 拼接完整路径（和列表页一致）
                            $("#ppicPreview").attr("src", fullPicUrl);    // 预览图回显
                            $("#ppic").val(jsonObj.pic);                  // 隐藏域赋值
                        } else {
                            $("#ppicPreview").attr("src", "../../img/gallery-photo/thumb-3.jpg");
                            $("#ppic").val("");
                        }
        ...
        function submitUpdateFile(){
            $("#updateAlbumForm").ajaxSubmit({
                ...
                success:function (json) {
                    // 上传成功后更新预览图和隐藏域
                    $("#ppicPreview").attr("src", json.realPath);
                    $("#ppic").val(json.relativePath);
                    layer.msg("封面上传成功！", {icon: 1});
                },
                error:function(){
...
<div class="wrapper">
    ...
    <div class="page-content">
        <div class="content container">
            <div class="row">
                <div class="col-lg-12">
                    <h2 class="page-title">专辑列表 <small>favor song</small></h2>
                </div>
            </div>
...
<%--                                                <td><img src="${pageContext.request.contextPath}${album.pic}" /></td>--%>
                                                <td> ${album.aname} </td>
...
<div id="albumPop1" style="margin-right: 50px;margin-top: 50px; display: none;">
    <form id="updateAlbumForm" class="layui-form" method="post" action="/album/updateAlbum" lay-filter="example">
        ...
        <div class="layui-form-item">
            <label class="layui-form-label" style="width:100px">专辑名字</label>
            <div class="layui-input-block">
                <input id="paname" type="text" name="aname" style="color: black; border-color: lightgray;background-color: white"
                       lay-verify="aname" autocomplete="off" placeholder="请输入专辑名" class="layui-input">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            <label class="layui-form-label" style="width:100px">发行公司</label>
            <div class="layui-input-block">
                <input id="pcompany" type="text" name="company" style="color: black; border-color: lightgray;background-color: white"
                       lay-verify="company" autocomplete="off" placeholder="请输入公司名" class="layui-input">
            </div>
        </div>

        <!-- 日期输入框ID为ppdate，匹配laydate渲染 -->
        <div class="layui-form-item">
            <label class="layui-form-label" style="width:100px">发行时间</label>
            <div class="layui-input-block">
                <input id="ppdate" type="text" name="pdate" style="color: black; border-color: lightgray;background-color: white"
                       lay-verify="pdate" autocomplete="off" placeholder="请选择发行时间" class="layui-input" readonly>
            </div>
        </div>

        <div class="layui-form-item">
            <label class="layui-form-label" style="width:100px">专辑封面</label>
            <div class="layui-input-block">
                <div style="margin-bottom: 10px;">
                    <img id="ppicPreview" src="../../img/gallery-photo/thumb-3.jpg" alt="专辑封面" width="150" height="150" style="border: 1px solid #eee;">
                </div>
                <input id="ppic" type="hidden" name="pic" lay-verify="pic">
                <button type="button" class="layui-btn layui-btn-sm layui-btn-normal" onclick="$('#updateI-file').click();">
                    重新上传封面
                </button>
                <input type="file" name="picfile" id="updateI-file" accept=".jpg, .png" onchange="submitUpdateFile()"
                       style="display: none">
            </div>
        </div>

        <div class="layui-form-item">
            <label class="layui-form-label" style="width:100px">语种</label>
            <div class="layui-input-block">
                <input id="plang" type="text" name="lang" style="color: black; border-color: lightgray;background-color: white"
                       lay-verify="lang" autocomplete="off" placeholder="请输入中文的语种" class="layui-input">
            </div>
        </div>
